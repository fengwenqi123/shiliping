      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <p class="nei" @click="openDetails(scope.row)">{{scope.row.loginName}}</p>
          </template>
        </el-table-column>
        <el-table-column
          prop="name"
  ...
  export default {
    ...
    methods: {
      ...
      getAuth: function () {
        var _this = this
        $.ajax({
          url: _this.apiUrl + 'account/?time=' + new Date().getTime(),
          type: 'GET',
          dataType: 'json',
          data: {
      ...
      modify() {
        ...
        // this.$router.push({
        //   path: '/sysModification_mod/' + this.mul[0].id + '/' + this.mul[0].loginName + '/' + this.mul[0].name + '/' + this.mul[0].mobile + '/' + this.mul[0].status + '/' + this.mul[0].description
        // })
      },
      // 删除
      del() {
        ...
        this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
          ...
          var _this = this
          $.ajax({
            url: _this.apiUrl + 'account/remove',
            type: 'PUT',
            dataType: 'json',
            data: {
              id: num
      ...
      Enable() {
        ...
        this.$confirm('此操作即将启动, 是否继续?', '提示', {
          ...
          var _this = this
          $.ajax({
            url: _this.apiUrl + 'account/enable',
            type: 'PUT',
            dataType: 'json',
            data: {
              id: num
      ...
      disEnable() {
        ...
        this.$confirm('此操作即将启动, 是否继续?', '提示', {
          ...
          var _this = this
          $.ajax({
            url: _this.apiUrl + 'account/disable',
            type: 'PUT',
            dataType: 'json',
            data: {
              id: num
